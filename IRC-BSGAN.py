...
parser.add_argument('--dataset', metavar='DATASET', default='cifar10',
                        choices=['cifar10','svhn','mnist','cinic','tiny','stl10','covid2','covid3'],
...
def train(trainloader,unlabelledloader, model, ema_model, optimizer, epoch, filep):
    ...
    for o,(u, _) in enumerate(unlabelledloader):
        ...
        C_fake_wei = torch.zeros(C_fake_wei.size(0), 3).cuda().scatter_(1, C_fake_wei, 1)
        ...
        loss = 0.1* C_fake_loss+C_unlabeled_loss+C_labeled_loss +class_loss#+mixup_consistency_loss+#+0.1*C_fake_lossmix+C_labeled_loss +C_unlabeled_loss+generated_weight(epoch)*C_u_lossmix
        meters.update('loss', loss.item())
        ...
        ema_prec1, ema_prec5 = accuracy(ema_logit_labeled.data, target_var.data, topk=(1, 3))
        ...
        if True:        
                ...
                D_loss = D_labeled_loss + D_unlabeled_loss + D_fake_loss+((L_reall1+L_realu2+L_D_fake1)*0.1)+L_dis*0.1+consistency_relation_distl*0.1+consistency_relation_distul*0.1+consistency_relation_distdf*0.1#self.train_hist['D_loss'].append(D_loss.item())
...
def validate(eval_loader, model, global_step, epoch, filep, ema = False, testing = False):
    ...
    for i, (input, target) in enumerate(eval_loader):
        ...
        prec1, prec5 = accuracy(output1.data, target_var.data, topk=(1, 3))
